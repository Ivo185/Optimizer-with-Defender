...
function Get-Localization {
    ...
    $lang = @{}
    if ($choice -eq "1") {
        $lang.Title = "ПАРАМЕТРИ ЗА ОПТИМИЗАЦИЯ"
        ...
        $lang.Title = "OPTIMIZATION PARAMETERS"
...
$global:logPath = Join-Path $PSScriptRoot "Optimization_Report.txt"
...
# 5. СЪЗДАВАНЕ НА ТОЧКА ЗА ВЪЗСТАНОВЯВАНЕ
...
if ($global:doScan) {
    ...
    try {
        # Добавяме -ErrorAction Stop, за да уловим грешката, ако вече има сканиране
        ...
        # Улавяме грешката и извеждаме чисто съобщение, без червен текст
